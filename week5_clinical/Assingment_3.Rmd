...
Before running this code, make sure you are in the correct working directory (your personal qbio_490_name folder) with getwd() and setwd(). Otherwise you’ll have trouble tracking the data down!
...
```{r setup}
    getwd()
    knitr::opts_knit$set(root.dir = normalizePath("/home1/zhade/490_cluster/analysis_data")) 
```
...
```{r Exercise 1.10}
newMtCars <- mtcars[1:31, c(1:8, 10:12)]
head(newMtCars)
```
...
```{r Exercise 4.1}
# create plot here
# Create a scatter plot in base R
...
```{r}
vital_status_na_mask <- clinic$vital_status != "NA"
```
...
```{r}
#no_middle_clinic <- c(vital_status_na_mask, male_mask, under30_over60_mask)
# Use unlink to remove a directory and its contents
unlink("/home1/zhade/R/x86_64-pc-linux-gnu-library/4.4/00LOCK-nloptr", recursive = TRUE)
...
p=0.016. It's statistically significant. A p-value of below 0.05 is singificant, which means there is a strong coorelation between age and breast cancer survival. 
